    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for field in embed.fields:
            if field.name == "🔖 Tags":
                ...
                if new_tag:
                    fields.append(
                        discord.EmbedField(name="🔖 Tags", value=new_tag, inline=True)
                    )
...
async def bookmark_tag(ctx, message: discord.Message):
    ...
    if message.guild:
        embed.add_field(name="🏰 Guild", value=f"[{message.guild.id}](https://discordlookup.com/guild/{message.guild.id})", inline=True)
    else:
